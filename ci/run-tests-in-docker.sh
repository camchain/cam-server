#!/bin/bash
#
# This script is run inside the Docker container and tests cam-server
#
set -e
...
expect /opt/ci/test-cam-server.expect

# Start cam-server in background for additional JSON-RPC tests
screen -dmS node1 bash -c "dotnet cam-server.dll --rpc"
